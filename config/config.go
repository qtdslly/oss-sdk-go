...
var defaultAWSConfigResolvers = []awsConfigResolver{
	// Resolves the default configuration the SDK's oss.Config will be
	// initialized with.
...
type awsConfigResolver func(ctx context.Context, cfg *oss.Config, configs configs) error
...
func (cs configs) ResolveAWSConfig(ctx context.Context, resolvers []awsConfigResolver) (oss.Config, error) {
	var cfg oss.Config
...
// This implements the oss.ConfigResolver interface.
...
func LoadDefaultConfig(ctx context.Context, optFns ...func(*LoadOptions) error) (cfg oss.Config, err error) {
