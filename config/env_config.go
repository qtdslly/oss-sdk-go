...
type EnvConfig struct {
	...
	//
	// Setting a custom HTTPClient in the oss.Config options will override this setting.
...
func (c EnvConfig) getDefaultsMode(ctx context.Context) (oss.DefaultsMode, bool, error) {
...
// valid value.
func (c EnvConfig) GetRetryMode(ctx context.Context) (oss.RetryMode, bool, error) {
...
func setDefaultsModeFromEnvVal(mode *oss.DefaultsMode, keys []string) error {
	for _, k := range keys {
...
func setRetryModeFromEnvVal(mode *oss.RetryMode, keys []string) (err error) {
	for _, k := range keys {
		if value := os.Getenv(k); len(value) > 0 {
			*mode, err = oss.ParseRetryMode(value)
			if err != nil {
...
// used for requests.
func (c EnvConfig) GetUseDualStackEndpoint(ctx context.Context) (value oss.DualStackEndpointState, found bool, err error) {
...
// used for requests.
func (c EnvConfig) GetUseFIPSEndpoint(ctx context.Context) (value oss.FIPSEndpointState, found bool, err error) {
...
func setEndpointDiscoveryTypeFromEnvVal(dst *oss.EndpointDiscoveryEnableState, keys []string) error {
	for _, k := range keys {
		value := os.Getenv(k)
		if len(value) == 0 {
...
func setUseDualStackEndpointFromEnvVal(dst *oss.DualStackEndpointState, keys []string) error {
	for _, k := range keys {
		value := os.Getenv(k)
		if len(value) == 0 {
			continue // skip if empty
...
func setUseFIPSEndpointFromEnvVal(dst *oss.FIPSEndpointState, keys []string) error {
	for _, k := range keys {
		value := os.Getenv(k)
		if len(value) == 0 {
			continue // skip if empty
...
func (c EnvConfig) GetEnableEndpointDiscovery(ctx context.Context) (value oss.EndpointDiscoveryEnableState, found bool, err error) {
