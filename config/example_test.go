...
func ExampleWithCredentialsCacheOptions() {
	cfg, err := config.LoadDefaultConfig(context.TODO(),
		config.WithCredentialsCacheOptions(func(o *oss.CredentialsCacheOptions) {
...
func ExampleWithEndpointResolver() {
	cfg, err := config.LoadDefaultConfig(context.TODO(),
		config.WithEndpointResolver(oss.EndpointResolverFunc(
			func(service, region string) (oss.Endpoint, error) {
				return oss.Endpoint{URL: "https://mock.amazonaws.com"}, nil
			})),
	)
...
func ExampleWithEndpointResolverWithOptions() {
	cfg, err := config.LoadDefaultConfig(context.TODO(),
		config.WithEndpointResolverWithOptions(oss.EndpointResolverWithOptionsFunc(
			func(service, region string, options ...interface{}) (oss.Endpoint, error) {
				return oss.Endpoint{URL: "https://mock.amazonaws.com"}, nil
			})),
	)
