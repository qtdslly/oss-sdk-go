...
type credentialsProviderProvider interface {
	getCredentialsProvider(ctx context.Context) (oss.CredentialsProvider, bool, error)
}

// getCredentialsProvider searches the configs for a credentialsProviderProvider
// and returns the value if found. Returns an error if a provider fails before a
// value is found.
func getCredentialsProvider(ctx context.Context, configs configs) (p oss.CredentialsProvider, found bool, err error) {
	for _, cfg := range configs {
...
// the oss.CredentialsCacheOptions.
type credentialsCacheOptionsProvider interface {
	getCredentialsCacheOptions(ctx context.Context) (func(*oss.CredentialsCacheOptions), bool, error)
}
...
func getCredentialsCacheOptionsProvider(ctx context.Context, configs configs) (
	f func(*oss.CredentialsCacheOptions), found bool, err error,
) {
...
// endpointResolverProvider is an interface for retrieving an oss.EndpointResolver from a configuration source
type endpointResolverProvider interface {
	getEndpointResolver(ctx context.Context) (oss.EndpointResolver, bool, error)
}

// getEndpointResolver searches the provided config sources for a EndpointResolverFunc that can be used
// to configure the oss.Config.EndpointResolver value.
func getEndpointResolver(ctx context.Context, configs configs) (f oss.EndpointResolver, found bool, err error) {
...
// endpointResolverWithOptionsProvider is an interface for retrieving an oss.EndpointResolverWithOptions from a configuration source
type endpointResolverWithOptionsProvider interface {
	getEndpointResolverWithOptions(ctx context.Context) (oss.EndpointResolverWithOptions, bool, error)
}

// getEndpointResolver searches the provided config sources for a EndpointResolverFunc that can be used
// to configure the oss.Config.EndpointResolver value.
func getEndpointResolverWithOptions(ctx context.Context, configs configs) (f oss.EndpointResolverWithOptions, found bool, err error) {
	for _, c := range configs {
...
// to configure the oss.Config.Logger value.
...
// clientLogModeProvider is an interface for retrieving the oss.ClientLogMode from a configuration source.
type clientLogModeProvider interface {
	getClientLogMode(ctx context.Context) (oss.ClientLogMode, bool, error)
}

func getClientLogMode(ctx context.Context, configs configs) (m oss.ClientLogMode, found bool, err error) {
	for _, c := range configs {
...
type retryProvider interface {
	getRetryer(ctx context.Context) (func() oss.Retryer, bool, error)
}

func getRetryer(ctx context.Context, configs configs) (v func() oss.Retryer, found bool, err error) {
	for _, c := range configs {
...
type defaultsModeProvider interface {
	getDefaultsMode(context.Context) (oss.DefaultsMode, bool, error)
}

func getDefaultsMode(ctx context.Context, configs configs) (v oss.DefaultsMode, found bool, err error) {
	for _, c := range configs {
...
type retryModeProvider interface {
	GetRetryMode(context.Context) (oss.RetryMode, bool, error)
}

func getRetryMode(ctx context.Context, configs configs) (v oss.RetryMode, found bool, err error) {
	for _, c := range configs {
