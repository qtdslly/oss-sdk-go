...
import (
	...
	"oss-sdk-go/service/sso"
	"oss-sdk-go/service/sts"
)
...
// and SharedConfig.
func resolveCredentials(ctx context.Context, cfg *oss.Config, configs configs) error {
...
func resolveCredentialProvider(ctx context.Context, cfg *oss.Config, configs configs) (bool, error) {
...
// credentials are only refreshed when needed. This also protects the
// credential provider to be used concurrently.
func resolveCredentialChain(ctx context.Context, cfg *oss.Config, configs configs) (err error) {
...
func resolveCredsFromProfile(ctx context.Context, cfg *oss.Config, envConfig *EnvConfig, sharedConfig *SharedConfig, configs configs) (err error) {
...
func resolveSSOCredentials(ctx context.Context, cfg *oss.Config, sharedConfig *SharedConfig, configs configs) error {
...
func processCredentials(ctx context.Context, cfg *oss.Config, sharedConfig *SharedConfig, configs configs) error {
...
func resolveLocalHTTPCredProvider(ctx context.Context, cfg *oss.Config, endpointURL, authToken string, configs configs) error {
...
func resolveHTTPCredProvider(ctx context.Context, cfg *oss.Config, url, authToken string, configs configs) error {
	...
	cfg.Credentials, err = wrapWithCredentialsCache(ctx, configs, provider, func(options *oss.CredentialsCacheOptions) {
...
func resolveCredsFromSource(ctx context.Context, cfg *oss.Config, envConfig *EnvConfig, sharedCfg *SharedConfig, configs configs) (err error) {
...
func resolveEC2RoleCredentials(ctx context.Context, cfg *oss.Config, configs configs) error {
...
func assumeWebIdentity(ctx context.Context, cfg *oss.Config, filepath string, roleARN, sessionName string, configs configs) error {
	if len(filepath) == 0 {
...
func credsFromAssumeRole(ctx context.Context, cfg *oss.Config, sharedCfg *SharedConfig, configs configs) (err error) {
	optFns := []func(*stscreds.AssumeRoleOptions){
		func(options *stscreds.AssumeRoleOptions) {
			...
			if len(sharedCfg.ExternalID) > 0 {
				options.ExternalID = oss.String(sharedCfg.ExternalID)
			}

			// Assume role with MFA
			if len(sharedCfg.MFASerial) != 0 {
				options.SerialNumber = oss.String(sharedCfg.MFASerial)
			}
		},
	}
...
// wrapWithCredentialsCache will wrap provider with an oss.CredentialsCache
// with the provided options if the provider is not already a
// oss.CredentialsCache.
func wrapWithCredentialsCache(
	ctx context.Context,
	cfgs configs,
	provider oss.CredentialsProvider,
	optFns ...func(options *oss.CredentialsCacheOptions),
) (oss.CredentialsProvider, error) {
	_, ok := provider.(*oss.CredentialsCache)
	if ok {
		return provider, nil
	}
	...
	return oss.NewCredentialsCache(provider, optFns...), nil
}
