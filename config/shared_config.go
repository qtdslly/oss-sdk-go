...
type SharedConfig struct {
	...
	//
	// Setting a custom HTTPClient in the oss.Config options will override this
...
func (c SharedConfig) getDefaultsMode(ctx context.Context) (value oss.DefaultsMode, ok bool, err error) {
...
func (c SharedConfig) GetRetryMode(ctx context.Context) (value oss.RetryMode, ok bool, err error) {
...
func (c SharedConfig) GetEnableEndpointDiscovery(ctx context.Context) (value oss.EndpointDiscoveryEnableState, ok bool, err error) {
	if c.EnableEndpointDiscovery == oss.EndpointDiscoveryUnset {
...
func (c SharedConfig) getCredentialsProvider() (oss.Credentials, bool, error) {
...
// used for requests.
func (c SharedConfig) GetUseDualStackEndpoint(ctx context.Context) (value oss.DualStackEndpointState, found bool, err error) {
	if c.UseDualStackEndpoint == oss.DualStackEndpointStateUnset {
...
// used for requests.
func (c SharedConfig) GetUseFIPSEndpoint(ctx context.Context) (value oss.FIPSEndpointState, found bool, err error) {
	if c.UseFIPSEndpoint == oss.FIPSEndpointStateUnset {
...
func updateDefaultsMode(mode *oss.DefaultsMode, section ini.Section, key string) error {
	if !section.Has(key) {
		return nil
	}
...
func updateRetryMode(mode *oss.RetryMode, section ini.Section, key string) (err error) {
	if !section.Has(key) {
		return nil
	}
	value := section.String(key)
	if *mode, err = oss.ParseRetryMode(value); err != nil {
		return err
	}
	return nil
}
...
func updateEndpointDiscoveryType(dst *oss.EndpointDiscoveryEnableState, section ini.Section, key string) {
	if !section.Has(key) {
		return
	}
	...
	case strings.EqualFold(value, endpointDiscoveryDisabled):
		*dst = oss.EndpointDiscoveryDisabled
	case strings.EqualFold(value, endpointDiscoveryEnabled):
		*dst = oss.EndpointDiscoveryEnabled
	case strings.EqualFold(value, endpointDiscoveryAuto):
		*dst = oss.EndpointDiscoveryAuto
	}
}

// updateEndpointDiscoveryType will only update the dst with the value in the section, if
// a valid key and corresponding EndpointDiscoveryType is found.
func updateUseDualStackEndpoint(dst *oss.DualStackEndpointState, section ini.Section, key string) {
	if !section.Has(key) {
		return
	}

	if section.Bool(key) {
		*dst = oss.DualStackEndpointStateEnabled
	} else {
		*dst = oss.DualStackEndpointStateDisabled
	}
...
// updateEndpointDiscoveryType will only update the dst with the value in the section, if
// a valid key and corresponding EndpointDiscoveryType is found.
func updateUseFIPSEndpoint(dst *oss.FIPSEndpointState, section ini.Section, key string) {
	if !section.Has(key) {
		return
	}

	if section.Bool(key) {
		*dst = oss.FIPSEndpointStateEnabled
	} else {
		*dst = oss.FIPSEndpointStateDisabled
	}
