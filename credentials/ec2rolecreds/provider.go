...
import (
	...
	sdkrand "oss-sdk-go/internal/rand"
...
func (p *Provider) Retrieve(ctx context.Context) (oss.Credentials, error) {
	...
	if len(credsList) == 0 {
		return oss.Credentials{Source: ProviderName},
...
func (p *Provider) HandleFailToRefresh(ctx context.Context, prevCreds oss.Credentials, err error) (
	...
	if err != nil {
		return oss.Credentials{}, fmt.Errorf("failed to get random float, %w", err)
	}
...
func (p *Provider) AdjustExpiresBy(creds oss.Credentials, dur time.Duration) (
	oss.Credentials, error,
) {
	if !creds.CanExpire {
		return creds, nil
