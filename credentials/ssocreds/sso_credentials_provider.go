...
func (p *Provider) Retrieve(ctx context.Context) (oss.Credentials, error) {
	if p.cachedTokenFilepath == "" {
		...
		if err != nil {
			return oss.Credentials{}, &InvalidTokenError{Err: err}
		}
	...
	if tokenFile.ExpiresAt == nil || sdk.NowTime().After(time.Time(*tokenFile.ExpiresAt)) {
		return oss.Credentials{}, &InvalidTokenError{}
	}
	...
	return oss.Credentials{
		AccessKeyID:     oss.ToString(output.RoleCredentials.AccessKeyId),
		SecretAccessKey: oss.ToString(output.RoleCredentials.SecretAccessKey),
		SessionToken:    oss.ToString(output.RoleCredentials.SessionToken),
		CanExpire:       true,
