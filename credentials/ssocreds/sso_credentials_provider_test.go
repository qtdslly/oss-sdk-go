...
func (m mockClient) GetRoleCredentials(ctx context.Context, params *sso.GetRoleCredentialsInput, optFns ...func(options *sso.Options)) (out *sso.GetRoleCredentialsOutput, err error) {
	...
	if len(m.ExpectedAccountID) > 0 {
		if diff := cmp.Diff(m.ExpectedAccountID, oss.ToString(params.AccountId)); len(diff) > 0 {
			m.t.Error(diff)
		}
	}

	if len(m.ExpectedAccessToken) > 0 {
		if diff := cmp.Diff(m.ExpectedAccessToken, oss.ToString(params.AccessToken)); len(diff) > 0 {
			m.t.Error(diff)
		}
	}

	if len(m.ExpectedRoleName) > 0 {
		if diff := cmp.Diff(m.ExpectedRoleName, oss.ToString(params.RoleName)); len(diff) > 0 {
			m.t.Error(diff)
		}
	}
...
func TestProvider(t *testing.T) {
	...
	cases := map[string]struct {
		...
		ExpectedErr         string
		ExpectedCredentials oss.Credentials
	}{
				...
				Response: func(mock mockClient) (*sso.GetRoleCredentialsOutput, error) {
					return &sso.GetRoleCredentialsOutput{
						RoleCredentials: &types.RoleCredentials{
							AccessKeyId:     oss.String("AccessKey"),
							SecretAccessKey: oss.String("SecretKey"),
							SessionToken:    oss.String("SessionToken"),
