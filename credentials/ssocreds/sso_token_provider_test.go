		...
		"refresh expired token": {
			...
			postRetrieve: func() error {
				...
				expect := token{
					tokenKnownFields: tokenKnownFields{
						...
						ExpiresAt:   (*rfc3339)(oss.Time(time.Date(2021, 12, 21, 12, 31, 1, 0, time.UTC))),
			...
			client: &mockCreateTokenAPIClient{
				expectInput: &ssooidc.CreateTokenInput{
					ClientId:     oss.String("client id"),
					ClientSecret: oss.String("client secret"),
					RefreshToken: oss.String("refresh token"),
					GrantType:    oss.String("refresh_token"),
				},
				output: &ssooidc.CreateTokenOutput{
					AccessToken:  oss.String("updated access token"),
					ExpiresIn:    600,
					RefreshToken: oss.String("updated refresh token"),
				},
			},
			expectToken: smithybearer.Token{
			...
			postRetrieve: func() error {
				...
				expect := token{
					tokenKnownFields: tokenKnownFields{
						...
						ExpiresAt:   (*rfc3339)(oss.Time(time.Date(2021, 12, 21, 12, 21, 1, 0, time.UTC))),
