...
//
//	cfg.Credentials = oss.NewCredentialsCache(creds)
//
...
//			o.SerialNumber = oss.String("myTokenSerialNumber")
...
func (p *AssumeRoleProvider) Retrieve(ctx context.Context) (oss.Credentials, error) {
	...
	input := &sts.AssumeRoleInput{
		DurationSeconds:   oss.Int32(int32(p.options.Duration / time.Second)),
		PolicyArns:        p.options.PolicyARNs,
		RoleArn:           oss.String(p.options.RoleARN),
		RoleSessionName:   oss.String(p.options.RoleSessionName),
	...
	if p.options.SerialNumber != nil {
		if p.options.TokenProvider != nil {
			...
			input.TokenCode = oss.String(code)
		} else {
			return oss.Credentials{}, fmt.Errorf("assume role with MFA enabled, but TokenProvider is not set")
		}
	}

	resp, err := p.options.Client.AssumeRole(ctx, input)
	if err != nil {
		return oss.Credentials{Source: ProviderName}, err
	}
