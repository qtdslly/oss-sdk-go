...
func (p *WebIdentityRoleProvider) Retrieve(ctx context.Context) (oss.Credentials, error) {
	b, err := p.options.TokenRetriever.GetIdentityToken()
	if err != nil {
		return oss.Credentials{}, fmt.Errorf("failed to retrieve jwt from provide source, %w", err)
	}
	...
	input := &sts.AssumeRoleWithWebIdentityInput{
		...
		WebIdentityToken: oss.String(string(b)),
	}
	if p.options.Duration != 0 {
		// If set use the value, otherwise STS will assign a default expiration duration.
		input.DurationSeconds = oss.Int32(int32(p.options.Duration / time.Second))
	}
	...
	if err != nil {
		return oss.Credentials{}, fmt.Errorf("failed to retrieve credentials, %w", err)
	}
	...
	value := oss.Credentials{
		AccessKeyID:     oss.ToString(resp.Credentials.AccessKeyId),
		SecretAccessKey: oss.ToString(resp.Credentials.SecretAccessKey),
		SessionToken:    oss.ToString(resp.Credentials.SessionToken),
