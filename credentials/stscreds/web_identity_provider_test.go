...
func TestWebIdentityProviderRetrieve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		expectedCredValue oss.Credentials
	}{
		"success": {
				...
				*sts.AssumeRoleWithWebIdentityOutput, error,
				...
				return &sts.AssumeRoleWithWebIdentityOutput{
					Credentials: &types.Credentials{
						Expiration:      oss.Time(sdk.NowTime()),
						AccessKeyId:     oss.String("access-key-id"),
						SecretAccessKey: oss.String("secret-access-key"),
						SessionToken:    oss.String("session-token"),
					},
				}, nil
			},
			expectedCredValue: oss.Credentials{
		...
		"success with duration and policy": {
			...
			options: func(o *stscreds.WebIdentityRoleOptions) {
				...
				o.Policy = oss.String("super secret policy")
				o.RoleSessionName = "foo"
			},
			mockClient: func(
				ctx context.Context, params *sts.AssumeRoleWithWebIdentityInput, optFns ...func(*sts.Options),
				...
				*sts.AssumeRoleWithWebIdentityOutput, error,
				...
				if e, a := int32(42), oss.ToInt32(params.DurationSeconds); e != a {
					return nil, fmt.Errorf("expect %v duration seconds, got %v", e, a)
				}
				if e, a := "super secret policy", oss.ToString(params.Policy); e != a {
