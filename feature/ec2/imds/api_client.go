...
import (
	...
	internalconfig "oss-sdk-go/feature/ec2/imds/internal/config"
...
func NewFromConfig(cfg oss.Config, optFns ...func(*Options)) *Client {
	opts := Options{
...
func resolveClientEnableState(cfg oss.Config, options *Options) error {
	if options.ClientEnableState != ClientDefaultEnableState {
		return nil
	}
...
func resolveEndpointModeConfig(cfg oss.Config, options *Options) error {
...
func resolveEndpointConfig(cfg oss.Config, options *Options) error {
	if len(options.Endpoint) != 0 {
		return nil
	}
