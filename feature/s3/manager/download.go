...
import (
	...
	"oss-sdk-go/oss"
	"oss-sdk-go/oss/middleware"
	"oss-sdk-go/internal/awsutil"
...
// downloads, or in memory []byte wrapper using oss.WriteAtBuffer. In case you download
...
func (d Downloader) Download(ctx context.Context, w io.WriterAt, input *s3.GetObjectInput, options ...func(*Downloader)) (n int64, err error) {
	if err := validateSupportedARNType(oss.ToString(input.Bucket)); err != nil {
		return 0, err
	}
...
func (d *downloader) download() (n int64, err error) {
	...
	if rng := oss.ToString(d.in.Range); len(rng) > 0 {
		d.downloadRange(rng)
...
func (d *downloader) downloadChunk(chunk dlchunk) error {
	...
	params.Range = oss.String(chunk.ByteRange())
