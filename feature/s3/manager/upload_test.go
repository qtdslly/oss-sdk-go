...
import (
	...
	s3testing "oss-sdk-go/feature/s3/manager/internal/testing"
...
func TestUploadOrderMulti(t *testing.T) {
	...
	resp, err := u.Upload(context.Background(), &s3.PutObjectInput{
		...
		Key:                  oss.String("Key - value"),
		Body:                 bytes.NewReader(buf12MB),
		ServerSideEncryption: "aws:kms",
		SSEKMSKeyId:          oss.String("KmsId"),
		ContentType:          oss.String("content/type"),
	})
	...
	// UploadPart
	for i := 1; i < 4; i++ {
		v := oss.ToString((*args)[i].(*s3.UploadPartInput).UploadId)
		if "UPLOAD-ID" != v {
	...
	v := oss.ToString((*args)[4].(*s3.CompleteMultipartUploadInput).UploadId)
	if "UPLOAD-ID" != v {
		t.Errorf("Expected %q, but received %q", "UPLOAD-ID", v)
	}
	...
	for i := 0; i < 3; i++ {
		...
		etag := oss.ToString(parts[i].ETag)
	...
	if e, a := "KmsId", oss.ToString(cmu.SSEKMSKeyId); e != a {
		t.Errorf("expect %q, got %q", e, a)
	}

	if e, a := "content/type", oss.ToString(cmu.ContentType); e != a {
		t.Errorf("expect %q, got %q", e, a)
	}
}
...
func TestUploadOrderSingle(t *testing.T) {
	...
	if e, a := "KmsId", oss.ToString(putObjectInput.SSEKMSKeyId); e != a {
		t.Errorf("expect %q, got %q", e, a)
	}

	if e, a := "content/type", oss.ToString(putObjectInput.ContentType); e != a {
...
func TestUploadOrderMultiFailure(t *testing.T) {
	...
	c.UploadPartFn = func(u *s3testing.UploadLoggingClient, _ *s3.UploadPartInput) (*s3.UploadPartOutput, error) {
		...
		return &s3.UploadPartOutput{ETag: oss.String(fmt.Sprintf("ETAG%d", u.PartNum))}, nil
	}
...
func TestSSE(t *testing.T) {
	...
	_, err := mgr.Upload(context.Background(), &s3.PutObjectInput{
		Bucket:               oss.String("Bucket"),
		Key:                  oss.String("Key"),
		SSECustomerAlgorithm: oss.String("AES256"),
		SSECustomerKey:       oss.String("foo"),
...
func createTempFile(t *testing.T, size int64) (*os.File, func(*testing.T), error) {
	file, err := ioutil.TempFile(os.TempDir(), oss.SDKName+t.Name())
	if err != nil {
		return nil, nil, err
	}
	filename := file.Name()
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			client := s3.New(s3.Options{
				EndpointResolver: s3testing.EndpointResolverFunc(func(region string, options s3.EndpointResolverOptions) (oss.Endpoint, error) {
					return oss.Endpoint{
						URL: server.URL,
					}, nil
				}),
...
func TestUploaderValidARN(t *testing.T) {
	cases := map[string]struct {
		...
		"accesspoint": {
			input: s3.PutObjectInput{
				Bucket: oss.String("arn:aws:s3:us-west-2:123456789012:accesspoint/myap"),
				Key:    oss.String("test-key"),
				Body:   bytes.NewReader([]byte("test body content")),
			},
		},
		"outpost accesspoint": {
			input: s3.PutObjectInput{
				Bucket: oss.String("arn:aws:s3-outposts:us-west-2:012345678901:outpost/op-1234567890123456/accesspoint/myaccesspoint"),
				Key:    oss.String("test-key"),
				Body:   bytes.NewReader([]byte("test body content")),
			},
		},
		"s3-object-lambda accesspoint": {
			input: s3.PutObjectInput{
				Bucket: oss.String("arn:aws:s3-object-lambda:us-west-2:123456789012:accesspoint/myap"),
				Key:    oss.String("test-key"),
				Body:   bytes.NewReader([]byte("test body content")),
			},
			wantErr: true,
