...
func (o Options) GetEndpointVariant() (v EndpointVariant) {
	if o.UseDualStackEndpoint == oss.DualStackEndpointStateEnabled {
		v |= DualStackVariant
	}
	if o.UseFIPSEndpoint == oss.FIPSEndpointStateEnabled {
		v |= FIPSVariant
	}
	return v
}
...
func (ps Partitions) ResolveEndpoint(region string, opts Options) (oss.Endpoint, error) {
	if len(ps) == 0 {
		return oss.Endpoint{}, fmt.Errorf("no partitions found")
	}
...
func (p Partition) ResolveEndpoint(region string, options Options) (resolved oss.Endpoint, err error) {
...
func (e Endpoint) resolve(partition, region string, def Endpoint, options Options) (oss.Endpoint, error) {
	var merged Endpoint
	merged.mergeIn(def)
	merged.mergeIn(e)
	e = merged
	...
	if e.IsZero() {
		return oss.Endpoint{}, fmt.Errorf("unable to resolve endpoint for region: %v", region)
	}

	var u string
	if e.Unresolveable != oss.TrueTernary {
	...
	if e.Deprecated == oss.TrueTernary && options.LogDeprecated {
...
func (e *Endpoint) mergeIn(other Endpoint) {
	if other.Unresolveable != oss.UnknownTernary {
	...
	if other.Deprecated != oss.UnknownTernary {
