...
type IsErrorRetryable interface {
	IsErrorRetryable(error) oss.Ternary
}
...
func (r IsErrorRetryables) IsErrorRetryable(err error) oss.Ternary {
	for _, re := range r {
		if v := re.IsErrorRetryable(err); v != oss.UnknownTernary {
			return v
		}
	}
...
type IsErrorRetryableFunc func(error) oss.Ternary

// IsErrorRetryable returns if the error is retryable.
func (fn IsErrorRetryableFunc) IsErrorRetryable(err error) oss.Ternary {
	return fn(err)
}
...
func (RetryableError) IsErrorRetryable(err error) oss.Ternary {
	...
	return oss.BoolTernary(v.RetryableError())
}
...
// canceled.
func (NoRetryCanceledError) IsErrorRetryable(err error) oss.Ternary {
...
func (r RetryableConnectionError) IsErrorRetryable(err error) oss.Ternary {
	if err == nil {
		return oss.UnknownTernary
	}
	var retryable bool
	...
	return oss.BoolTernary(retryable)
...
// HTTP status code.
func (r RetryableHTTPStatusCode) IsErrorRetryable(err error) oss.Ternary {
...
func (r RetryableErrorCode) IsErrorRetryable(err error) oss.Ternary {
