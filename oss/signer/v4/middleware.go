...
type SignHTTPRequestMiddlewareOptions struct {
	CredentialsProvider oss.CredentialsProvider
	Signer              HTTPSigner
	LogSigning          bool
}
...
func haveCredentialProvider(p oss.CredentialsProvider) bool {
	if p == nil {
		return false
	}
	switch p.(type) {
	case oss.AnonymousCredentials,
		*oss.AnonymousCredentials:
		return false
	}
