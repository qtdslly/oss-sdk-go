...
import (
	...
	"oss-sdk-go/service/eventbridge/internal/endpoints"
...
type EndpointResolver interface {
	ResolveEndpoint(region string, options endpoints.Options) (oss.Endpoint, error)
}
...
func (u *updateEndpoint) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (out middleware.SerializeOutput, metadata middleware.Metadata, err error) {
	...
	if u.endpointResolverOptions.UseFIPSEndpoint == oss.FIPSEndpointStateEnabled {
		return out, metadata, &smithy.SerializationError{
	...
	if endpoint.Source == oss.EndpointSourceCustom {
		return next.HandleSerialize(ctx, in)
	}
