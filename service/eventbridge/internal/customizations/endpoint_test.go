...
import (
	...
	"oss-sdk-go/service/eventbridge"
...
func TestPutEventsUpdateEndpoint(t *testing.T) {
	tests := map[string]struct {
		...
		UseDualStack     oss.DualStackEndpointState
		UseFIPS          oss.FIPSEndpointState
		EndpointId       *string
		Region           string
		WantErr          bool
		WantEndpoint     string
		...
		"dualstack aws endpoint": {
			Region:           "us-mock-1",
			UseDualStack:     oss.DualStackEndpointStateEnabled,
		...
		"fips aws endpoint": {
			Region:           "us-mock-1",
			UseFIPS:          oss.FIPSEndpointStateEnabled,
		...
		"multi-region aws endpoint": {
			Region:           "us-mock-1",
			EndpointId:       oss.String("abc123.456def"),
		...
		"multi-region custom endpoint service source": {
			...
			CustomEndpoint: &oss.Endpoint{
				URL:           "https://custom.amazonaws.com",
				SigningRegion: "us-mock-1",
				Source:        oss.EndpointSourceServiceMetadata,
			},
			WantEndpoint:     "https://abc123.456def.endpoint.events.amazonaws.com/",
			WantSignedName:   "events",
			WantSignedRegion: "*",
		...
		"multi-region aws endpoint empty endpoint id": {
			Region:     "us-mock-1",
			EndpointId: oss.String(""),
			WantErr:    true,
		},
		"multi-region aws endpoint bad host label": {
			Region:     "us-mock-1",
			EndpointId: oss.String("badactor.com?foo=bar"),
			WantErr:    true,
		},
		...
		t.Run(name, func(t *testing.T) {
			...
			if tt.CustomEndpoint != nil {
				options.EndpointResolver = eventbridge.EndpointResolverFunc(
					func(region string, options eventbridge.EndpointResolverOptions) (oss.Endpoint, error) {
