...
type DiscoverEndpoint struct {
	...
	// discovery feature.
	EndpointDiscoveryEnableState oss.EndpointDiscoveryEnableState
...
func (d *DiscoverEndpoint) HandleSerialize(
	...
	if d.EndpointDiscoveryEnableState == oss.EndpointDiscoveryDisabled {
		return next.HandleSerialize(ctx, in)
	}

	// if operation does not require endpoint discovery, and endpoint discovery is not explicitly enabled,
	// skip this workflow
	if !d.EndpointDiscoveryRequired && d.EndpointDiscoveryEnableState != oss.EndpointDiscoveryEnabled {
		return next.HandleSerialize(ctx, in)
	}

	// when custom endpoint is provided
	if es := awsmiddleware.GetEndpointSource(ctx); es == oss.EndpointSourceCustom {
		// error if endpoint discovery was explicitly enabled
		if d.EndpointDiscoveryEnableState == oss.EndpointDiscoveryEnabled {
