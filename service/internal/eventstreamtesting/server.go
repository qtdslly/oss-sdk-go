...
func setupServer(server *httptest.Server) oss.HTTPClient {
...
func SetupEventStream(
	...
	cfg oss.Config, cleanupFn func(), err error,
) {
	...
	cfg.EndpointResolver = oss.EndpointResolverFunc(func(service, region string) (oss.Endpoint, error) {
		return oss.Endpoint{URL: server.URL}, nil
	})
