...
import (
	...
	"oss-sdk-go/service/internal/integrationtest/s3shared"
...
func TestMain(m *testing.M) {
	...
	s3cfg := cfg.Copy()
	if len(s3Endpoint) != 0 {
		s3cfg.EndpointResolver = oss.EndpointResolverFunc(func(service, region string) (oss.Endpoint, error) {
			return oss.Endpoint{
				URL:           s3Endpoint,
				PartitionID:   "aws",
				SigningName:   "s3",
	...
	if len(s3ControlEndpoint) != 0 {
		s3ControlCfg.EndpointResolver = oss.EndpointResolverFunc(func(service, region string) (oss.Endpoint, error) {
			return oss.Endpoint{
...
func testCopyObject(t *testing.T, sourceBucket, targetBucket string, opts func(options *s3.Options)) {
	...
	_, err = s3client.CopyObject(context.Background(),
		&s3.CopyObjectInput{
			Bucket:     &targetBucket,
			Key:        &key,
			CopySource: oss.String("/" + sourceBucket + "/" + key),
		}, opts)
	if err != nil {
		t.Fatalf("expect no error, got %v", err)
	}
