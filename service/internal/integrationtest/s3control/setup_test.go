...
func TestMain(m *testing.M) {
	...
	if len(accountID) == 0 {
		...
		if len(stsEndpoint) != 0 {
			opts = func(options *sts.Options) {
				options.EndpointResolver = sts.EndpointResolverFunc(func(region string, options sts.EndpointResolverOptions) (oss.Endpoint, error) {
					return oss.Endpoint{
						URL:           stsEndpoint,
						PartitionID:   "aws",
						SigningName:   "sts",
	...
	if len(s3ControlEndpoint) != 0 {
		s3controlOpts = func(options *s3control.Options) {
			options.EndpointResolver = s3control.EndpointResolverFunc(func(region string, options s3control.EndpointResolverOptions) (oss.Endpoint, error) {
				return oss.Endpoint{
