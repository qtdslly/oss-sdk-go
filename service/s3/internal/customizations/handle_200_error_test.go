...
func (fn EndpointResolverFunc) ResolveEndpoint(region string, options s3.EndpointResolverOptions) (endpoint oss.Endpoint, err error) {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			options := s3.Options{
				...
				Region:      "mock-region",
				EndpointResolver: EndpointResolverFunc(func(region string, options s3.EndpointResolverOptions) (e oss.Endpoint, err error) {
					e.URL = server.URL
			...
			resp, err := svc.CompleteMultipartUpload(context.Background(), &s3.CompleteMultipartUploadInput{
				UploadId:     oss.String("mockID"),
				RequestPayer: "requester",
				Bucket:       oss.String("bucket"),
				Key:          oss.String("mockKey"),
			})
