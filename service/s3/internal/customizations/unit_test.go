...
func Test_EmptyResponse(t *testing.T) {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			cfg := oss.Config{
				Region: "us-east-1",
				EndpointResolver: oss.EndpointResolverFunc(func(service, region string) (oss.Endpoint, error) {
					return oss.Endpoint{
						URL:         server.URL,
						SigningName: "s3",
					}, nil
			...
			params := &s3.HeadBucketInput{Bucket: oss.String("aws-sdk-go-data")}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			cfg := oss.Config{
				...
				Retryer: func() oss.Retryer { return oss.NopRetryer{} },
			}
			...
			params := &s3.GetBucketLocationInput{
				Bucket: oss.String("aws-sdk-go-data"),
			}
