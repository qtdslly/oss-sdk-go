...
func TestWriteGetObjectResponse(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"Content-Length seekable": {
			...
			Input: s3.WriteGetObjectResponseInput{
				RequestRoute: oss.String("route"),
				RequestToken: oss.String("token"),
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			...
			client := s3.New(s3.Options{
				...
				EndpointResolver: s3.EndpointResolverFunc(func(region string, options s3.EndpointResolverOptions) (oss.Endpoint, error) {
					return oss.Endpoint{
						URL:               server.URL,
